...
save output/data/tfp_growth, replace

*bysort industry: egen mean_weighted_tfp_g_`full_start_yr'_20$end_year= mean(weighted_tfp_g_`full_start_yr'_20$end_year)
*label variable mean_weighted_tfp_g_`full_start_yr'_20$end_year "Average TFP growth`full_start_yr'-20$end_year weighed by VA"
...
*Calculate the average TFP growth for each industry (UHS data)

collapse tfp_growth, by (year industry)
rangestat (mean) tfp_growth, interval(year -4 0) by(industry)
